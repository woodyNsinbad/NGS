...
GetOptions(
	...
	say "     cut_fasta_new : cut fasta file into small ones ";
	...
	say "     head : select top n seq";
	say "     rename : rename the any sequence title with adding prefix";
	say "     N_cont : return N number in the fasta file  \n\n";
...
case "head"								{head($acc,@file)}
case "cut_fasta_new"							{cut_fasta_file_new($cutofflenth,$outfile,@file)}
case "N_cont"							{N_cont(@file)}
...
sub N_cont{
	...
	while(my $seqobj = $in->next_seq() ){
		...
		$totN +=() = $seqobj->seq =~ m/N/gi ;
	}

	say join "\t",($totlength,$totN, sprintf("%.2f",($totN)*100/($totlength))."%");
}


sub cut_fasta_file_new{
	my $cut_pieces_num = shift;
	my $od = shift;
	my $file = shift;
	my $in;
	 if( defined $od && -d $od){
	...
	my $all_seq_num = `grep -c ">" $file`;
	my $one_pieces = int( $all_seq_num / $cut_pieces_num);
	...
	my $file_name = basename($file);
	 foreach my $index ( 1 .. $cut_pieces_num ){
		system("touch $od/$file_name.$index");
	 }
	 
	 my $i = 1; 
	 my $out = Bio::SeqIO->new(-format => "fasta" , -file => ">$od/$file_name.1");
	 my $index = 1;
	 ...
	 while(my $seqobj = $in->next_seq() ){
		...
		if($i <= $one_pieces ){
			...
			$index++ ; 
			$out = Bio::SeqIO->new(-format => "fasta" , -file => ">$od/$file_name.$index");
			$out->write_seq($seqobj);
			$i = 1;
		}
	 }
